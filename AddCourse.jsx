...
const AddCourse = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alert(
        "An error occurred while creating the course, only .mp4 videos can be uploaded"
      );
    }
  };
  ...
  return (
    <div className="">
      <Form className="mb-3" onSubmit={handleSubmit}>
        ...
        <Form.Group className="mb-3" controlId="formGridEducator">
        ...
        {addCourse.sections.map((section, index) => (
          <div
            key={index}
            className="mb-4 p-3 border rounded-3 position-relative bg-white shadow-sm"
          >
            <i
              className="bi bi-x-circle-fill text-danger position-absolute top-0 end-0 p-2"
        ...
        <Row className="mb-3">
          <Col xs={24} md={12} lg={8}>
            <Button size="sm" variant="outline-success" onClick={addInputGroup}>
              Add Section
            </Button>
          </Col>
        </Row>
